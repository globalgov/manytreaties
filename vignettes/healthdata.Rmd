...
Extending from data in the agreements datacube, the HUGGO_MEM dataset in the `memberships` datacube contains hand-coded data of 39,000 observations on state memberships to international health instruments.
...
manytreaties::memberships$HUGGO_MEM[,c(1:5, 7:10, 13, 40)] %>% 
  dplyr::filter(grepl("Health", Domain)) %>%
...
formal <- manytreaties::agreements$HUGGO %>%
  ...
  dplyr::distinct()
net <- manytreaties::memberships$HUGGO_MEM %>%
  dplyr::filter(grepl("Health", Domain)) %>%
