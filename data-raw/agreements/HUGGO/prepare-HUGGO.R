...
#                         TypeAmbit = dplyr::recode(L, B = "Bilateral",
#                                                   M = "Multilateral"),
#                         TypeAgree = dplyr::recode(T, G = "A", M = "E", "T" = "Q",
#                                                   D = "V", R = "W", N = "X",
#                                                   U = "Y"),
#                         TypeGeo = dplyr::recode(J, G = "Global", R = "Regional",
#                                                   L = "Bilateral"),
#                         gengID = GENG) %>%
#   dplyr::mutate(Title = manytreaties::standardise_titles(Title)) %>%
#   dplyr::mutate(Begin = dplyr::coalesce(Signature, Force)) %>%
#   dplyr::select(gengID, Title, Begin, End, TypeAmbit, TypeAgree, TypeGeo, 
#                 Signature, Force) %>%
#   dplyr::arrange(Begin)
# HUGGO1$treatyID <- manytreaties::code_agreements(HUGGO1)
# 
# # Clean HUGGO2 ####
...
#                 Signature, Force) %>%
#   dplyr::arrange(Begin)
# HUGGO2$treatyID <- manytreaties::code_agreements(HUGGO2)
# 
# # Clean HUGGO4 ####
...
#   dplyr::mutate(Title = manytreaties::standardise_titles(Title)) %>%
#   dplyr::mutate(Begin = dplyr::coalesce(Signature, Force)) %>%
#   dplyr::select(gengID, Title, Begin, Signature, Force, End, Parties)
# HUGGO4$treatyID <- manytreaties::code_agreements(HUGGO4)
# 
# # Clean HUGGO6 ####
...
#   dplyr::distinct() %>%
#   dplyr::arrange(Begin)
# HUGGO6$treatyID <- manytreaties::code_agreements(HUGGO6)
# 
...
# 
# # Join the datasets together
# HUGGO_env <- manydata::consolidate(HUGGO_env, join = "full", 
